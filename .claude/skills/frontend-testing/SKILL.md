...
Playwright ships three built-in AI agents that automate E2E test creation and maintenance via an MCP server.

### Three-Agent Workflow

| Agent     | Role                                                                   |
| --------- | ---------------------------------------------------------------------- |
| Planner   | Reads the app + seed file, produces a structured test plan in `specs/` |
| Generator | Takes a single test case from the plan and writes a `.spec.ts` file    |
| Healer    | Runs failing tests and auto-fixes them                                 |
...
- **`specs/`** — Directory for markdown test plans generated by the planner.
- **`.claude/agents/`** — Agent definitions (do not modify; regenerated by `npx playwright init-agents`).
- **`.claude/prompts/`** — Prompt templates for invoking the agents.

### Invoking via Prompt Templates

Use the prompt templates in `.claude/prompts/`:

- **`playwright-test-plan`** — Plan tests for a feature
- **`playwright-test-generate`** — Generate tests from a plan
- **`playwright-test-heal`** — Fix failing tests
- **`playwright-test-coverage`** — Full pipeline: plan, generate, then heal

### Coexistence with Hand-Written Tests

Hand-written E2E tests (`*-flow.spec.ts`) are maintained manually and are **not touched by agents**. Agent-generated tests live in separate files (typically under `e2e/<feature>/`). Both sets run together with `npm run test:e2e`.
...
npm run test:e2e:mcp      # Start the Playwright MCP server for agentic testing
